...
A manpage is generated at build time and included in release archives. Install it system-wide:
```bash
sudo make install          # installs binary + manpage to /usr/local
man nv-swaptop
```
Or read it directly from a release tarball:
```bash
man ./nv-swaptop.1
```
